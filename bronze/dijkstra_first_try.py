...
def get_route(world_map, base, target, route_type: str = "shortest")->tuple:
    ...
    route = []
    no_option_count = 0
    for _ in range(3*route_distance):
        options = [
            (base[0]+1, base[1]),
            (base[0]-1, base[1]), 
            (base[0], base[1]+1),
        ...
        for option in options:
            ...
            and option not in route:
                ...
                if option_type not in (town_type, ink_type):
                    clean_options += [option]

        if not clean_options:
            if no_option_count<len(route):
                base = route[-no_option_count]
                no_option_count+=1
            continue
        ...
        shortest_option_points = world_map[shortest_option[1]][shortest_option[0]]["paint_points"]
        for option in clean_options:
            ...
            if new_remaining_distance <= remaining_distance:
                if option_paint_points <= shortest_option_points:
                    shortest_option = option
                    shortest_option_points = option_paint_points
                    remaining_distance = new_remaining_distance

        route += [shortest_option]
        base = shortest_option

        route_points += shortest_option_points

    return route, route_points

def get_options(world_map, base, route):
...
def get_dijkstra_route(world_map, base, target) -> tuple:
    """
    Use an algorithm based on the dijkstra idea of shortest path
    """
    route_points = 0
    distance = 0
    option_routes_heap = []
    # if the route only include the base it will have distance 0
    route = [base]
    heapq.heappush(option_routes_heap, (distance, route))
    complete_route = False
    while not complete_route:
        distance += 1
        options = get_options(world_map, base, route)
        if target in options:
            # found shortest path
            complete_route = True
        else:
            if len(options)>0:
                for option in options:
                    heapq.heappush(option_routes_heap, (distance, route + [option]))
            
            # If no options, we just check the next option in the heap
            distance, route = heapq.heappop(option_routes_heap)

            # the base will be the last added node to the route
            print(f"route {route}", file=sys.stderr, flush=True)
            base = route[-1]
...
for town_id, town_data in list(towns.items()):
    ...
    if "x" not in base_town["desired_connections"]:
        for target_town_id in base_town["desired_connections"]:
            ...
            towns[town_id]["routes"][(base,target)], towns[town_id]["scores"][(base, target)] = get_dijkstra_route(_map, base, target)
            desired_connections += [(town_id, target_town_id)]
...
while True:
    ...
    for i in range(height):
        for j in range(width):
            ...
            for connection in part_of_active_connections.split(","):
                track_importance += 1
                active_connections_tracks += [(j,i)]
                #connection_base, connection_target = connection.split("-")
                #active_connections += [(int(connection_base), int(connection_target))]
    ...
    for town_id, town_data in list(towns.items()):
        ...
        for target_town_id in town_data["desired_connections"]:
            ...
            towns[town_id]["routes"][(base, target)], \
            towns[town_id]["scores"][(base, target)] = \
            get_dijkstra_route(_map, base, target)
    ...
    for point in route:
        ...
        and point not in inked_tracks:
            ...
            my_regions += [_map[point[1]][point[0]]["region_id"]]
